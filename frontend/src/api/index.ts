 ...
 * - 500: Server error
 */
export async function postCookie(cookie: string, label?: string) {
  ...
  const response = await fetch("/api/cookie", {
    ...
    body: JSON.stringify({ cookie, ...(label !== undefined ? { label } : {}) }),
  });
...
/**
 * Updates the user-facing label for an existing cookie.
 */
export async function updateCookieLabel(cookie: string, label: string) {
  const token = localStorage.getItem("authToken") || "";
  const response = await fetch(`/api/cookie`, {
    method: "PUT",
    headers: {
