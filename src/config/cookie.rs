...
pub struct CookieStatus {
    ...
    /// User-facing label for identifying which Claude account this cookie belongs to.
    /// This is intentionally user-editable because Claude does not always expose a
    /// stable quota/account endpoint for every cookie state.
    #[serde(default)]
    pub label: Option<String>,
    /// Best-effort account email learned from Claude bootstrap after the cookie is used.
    #[serde(default)]
    pub account_email: Option<String>,
    /// Best-effort organization UUID learned from Claude bootstrap after the cookie is used.
    #[serde(default)]
    pub organization_uuid: Option<String>,
    /// Best-effort capability list learned from Claude bootstrap after the cookie is used.
    #[serde(default)]
