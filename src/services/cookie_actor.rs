...
impl CookieActor {
    ...
    fn apply_cookie_metadata(existing: &mut CookieStatus, incoming: &CookieStatus) {
        if incoming.label.is_some() {
            existing.label = Self::clean_label(incoming.label.clone());
        }
        if incoming.account_email.is_some() {
            existing.account_email = incoming.account_email.clone();
        }
        if incoming.organization_uuid.is_some() {
            existing.organization_uuid = incoming.organization_uuid.clone();
        }
        if !incoming.capabilities.is_empty() {
            existing.capabilities = incoming.capabilities.clone();
        }
        if incoming.supports_claude_1m_sonnet.is_some() {
            existing.supports_claude_1m_sonnet = incoming.supports_claude_1m_sonnet;
        }
        if incoming.supports_claude_1m_opus.is_some() {
            existing.supports_claude_1m_opus = incoming.supports_claude_1m_opus;
        }
    }
    ...
    fn collect(state: &mut CookieActorState, mut cookie: CookieStatus, reason: Option<Reason>) {
        ...
        match reason {
            ...
            Reason::Free => {
                ...
                if !state
                    .invalid
                    .insert(UselessCookie::from_cookie_status(&removed, reason))
                {
                    return;
                }
            }
    ...
    fn accept(state: &mut CookieActorState, mut cookie: CookieStatus) {
        cookie.label = Self::clean_label(cookie.label.clone());

        if let Some(existing) = state.valid.iter_mut().find(|c| **c == cookie) {
            Self::apply_cookie_metadata(existing, &cookie);
            Self::save(state);
            Self::log(state);
            return;
        }
        ...
        if !state.exhausted.is_empty() {
            ...
            for mut existing in state.exhausted.drain() {
                if existing == cookie {
                    Self::apply_cookie_metadata(&mut existing, &cookie);
                    updated = true;
                }
        ...
        if !state.invalid.is_empty() {
            ...
            for mut existing in state.invalid.drain() {
                if existing == cookie {
                    existing.label = cookie.label.clone();
                    existing.account_email = cookie.account_email.clone();
                    existing.organization_uuid = cookie.organization_uuid.clone();
                    existing.capabilities = cookie.capabilities.clone();
                    updated = true;
                }
                new_invalid.insert(existing);
            }
            state.invalid = new_invalid;
            if updated {
                warn!("Cookie already exists in invalid list; metadata updated");
                Self::save(state);
                Self::log(state);
                return;
            }
