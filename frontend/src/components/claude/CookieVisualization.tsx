...
const CookieVisualization: React.FC = () => {
  ...
  const [editingLabelCookie, setEditingLabelCookie] = useState<string | null>(
    null
  );
  ...
  const getAccountDisplay = (status: CookieItem) => {
    const manualLabel = status.label?.trim();
    const email = status.account_email?.trim();
    return manualLabel || email || (t("cookieStatus.account.unnamed") as string);
  };

  const getPlanDisplay = (status: CookieItem) => {
    const capabilities = status.capabilities || [];
    if (capabilities.some((c) => c.includes("max"))) return "Max";
    if (capabilities.some((c) => c.includes("raven"))) return "Raven";
    if (capabilities.some((c) => c.includes("enterprise"))) return "Enterprise";
    if (capabilities.some((c) => c.includes("pro"))) return "Pro";
    return null;
  };

  const quotaItems = (status: CookieItem) => {
    const items: Array<{ label: string; value: number }> = [];
    if (typeof status.session_utilization === "number") {
      items.push({
        label: t("cookieStatus.quota.session") as string,
        value: status.session_utilization,
      });
    }
    if (typeof status.seven_day_utilization === "number") {
      items.push({
        label: t("cookieStatus.quota.sevenDay") as string,
        value: status.seven_day_utilization,
      });
    }
    if (typeof status.seven_day_sonnet_utilization === "number") {
      items.push({
        label: t("cookieStatus.quota.sevenDaySonnet") as string,
        value: status.seven_day_sonnet_utilization,
      });
    }
    if (typeof status.seven_day_opus_utilization === "number") {
      items.push({
        label: t("cookieStatus.quota.sevenDayOpus") as string,
        value: status.seven_day_opus_utilization,
      });
    }
    return items;
  };
  ...
  const updateLocalCookieLabel = (cookie: string, label: string) => {
    const normalizedLabel = label.trim() || null;
    const updateList = <T extends CookieItem>(items: T[]) =>
      items.map((item) =>
        item.cookie === cookie ? { ...item, label: normalizedLabel } : item
      );

    setCookieStatus((prev) => ({
      valid: updateList(prev.valid) as CookieStatus[],
      exhausted: updateList(prev.exhausted) as CookieStatus[],
      invalid: updateList(prev.invalid) as typeof prev.invalid,
    }));
  };

  const handleStartEditLabel = (status: CookieItem) => {
    setEditingLabelCookie(status.cookie);
    setLabelDrafts((prev) => ({
      ...prev,
      [status.cookie]: status.label || "",
    }));
  };

  const handleSaveLabel = async (cookie: string) => {
    const label = labelDrafts[cookie] || "";
    setUpdatingCookie((prev) => ({ ...prev, [cookie]: true }));
    setError(null);
    try {
      const response = await updateCookieLabel(cookie, label);
      if (!response.ok) {
        const message =
          response.status === 401
      ...
      updateLocalCookieLabel(cookie, label);
      setEditingLabelCookie(null);
    } catch (err) {
      const message = err instanceof Error ? err.message : String(err);
      setError(message);
    } finally {
  ...
  const renderAccountSummary = (
    ...
    const cookieKey = status.cookie;
    const isEditing = editingLabelCookie === cookieKey;
    const saving = !!updatingCookie[cookieKey];
    const plan = getPlanDisplay(status);
    const quotas = quotaItems(status);
    const accentClass =
      color === "green"
    ...
    return (
      <div className={`mb-2 rounded-lg border p-2 ${accentClass}`}>
        <div className="flex flex-wrap items-center gap-2 text-xs">
          <span className="font-medium text-gray-300">
            {t("cookieStatus.account.title")}
          </span>
          {isEditing ? (
            <>
              <input
                value={labelDrafts[cookieKey] || ""}
                onChange={(e) =>
                  setLabelDrafts((prev) => ({
                    ...prev,
                    [cookieKey]: e.target.value,
                  }))
                }
                placeholder={t("cookieStatus.account.placeholder")}
                className="min-w-[220px] rounded border border-gray-600 bg-gray-900 px-2 py-1 text-gray-100 outline-none focus:border-cyan-400"
                disabled={saving}
              />
              <button
                type="button"
                onClick={() => handleSaveLabel(cookieKey)}
                disabled={saving}
                className="rounded bg-cyan-600 px-2 py-1 text-white hover:bg-cyan-500 disabled:opacity-50"
              >
                {saving
                  ? t("cookieStatus.context.saving")
                  : t("cookieStatus.account.save")}
              </button>
              <button
                type="button"
                onClick={() => setEditingLabelCookie(null)}
                disabled={saving}
                className="rounded bg-gray-700 px-2 py-1 text-gray-200 hover:bg-gray-600 disabled:opacity-50"
              >
                {t("cookieStatus.account.cancel")}
              </button>
            </>
          ) : (
            <>
              <span className="rounded bg-black/25 px-2 py-1 font-semibold text-white">
                {getAccountDisplay(status)}
              </span>
              {status.account_email && status.label && (
                <span className="text-gray-400">
                  {t("cookieStatus.account.email")}: {status.account_email}
                </span>
              )}
              {plan && (
                <span className="rounded bg-black/20 px-2 py-1 text-gray-200">
                  {plan}
                </span>
              )}
              <button
                type="button"
                onClick={() => handleStartEditLabel(status)}
                className="rounded border border-gray-600 px-2 py-1 text-gray-300 hover:border-cyan-400 hover:text-cyan-200"
              >
                {t("cookieStatus.account.edit")}
              </button>
            </>
          )}
        </div>
        ...
        <div className="mt-2 flex flex-wrap gap-2 text-xs">
          {quotas.length > 0 ? (
            quotas.map((item) => (
              <span key={item.label} className="rounded bg-black/25 px-2 py-1">
                {item.label}: {item.value}%
              </span>
            ))
          ) : (
            <span className="text-gray-400">
              {t("cookieStatus.account.noQuota")}
            </span>
          )}
        </div>
      </div>
            ...
            return (
              ...
              >
                <div className="text-green-300 flex-grow mr-4 min-w-0 mb-1 sm:mb-0">
                  {renderAccountSummary(status, "green")}
            ...
            return (
              ...
              >
                <div className="text-yellow-300 flex-grow mr-4 min-w-0 mb-1 sm:mb-0">
                  {renderAccountSummary(status, "yellow")}
                  <CookieValue cookie={status.cookie} />
                  {hasMeta && (
                    <details className="mt-1 text-xs text-gray-400">
                      <summary className="cursor-pointer text-gray-500 hover:text-gray-300">
            ...
            return (
              ...
              >
                <div className="text-red-300 flex-grow mr-4 min-w-0 mb-1 sm:mb-0">
                  {renderAccountSummary(status, "red")}
                  <CookieValue cookie={status.cookie} />
